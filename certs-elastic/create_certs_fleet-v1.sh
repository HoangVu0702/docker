...
echo "                             © 2023 DUDE. All rights reserved."
echo "--------------------------------------------------------------------------------------------------------"
# Create Container
...
if [[ $answer == "Y" || $answer == "y" ]]; then
    ...
    if [[ $(echo "$file_list" | wc -l) -eq 2 && $(echo "$file_list" | uniq | wc -l) -eq 1 ]]; then
        ca_file_name=$(echo "$file_list" | head -1)
        ...
        --name fleet-server \
        --ca-cert $ca_folder_name/$ca_file_name \
        --ca-key $ca_folder_name/$ca_file_name \
        --dns $hostname_fleet \
        --ip $ip_fleet \
        ...
        "bin/elasticsearch-certutil cert \
        --name fleet-server \
        --ca-cert $ca_folder_name/$ca_file_name_crt \
        --ca-key $ca_folder_name/$ca_file_name_key \
        --dns $hostname_fleet \
        --ip $ip_fleet \
        --pem --out /usr/share/elasticsearch/$certs_file_name"
        # Chain 4 Copy all files to the host
        docker cp es-create-certs:/usr/share/elasticsearch/$certs_file_name .
        docker cp es-create-certs:/usr/share/elasticsearch/$ca_file_name .
        echo "Certificates created and copied to the host."
    fi
    ...
    docker exec -it $(docker ps -aq --filter name=es-create-certs) bash -c "bin/elasticsearch-certutil ca --pem -out $ca_folder.zip && unzip $ca_file_name"
    read -p "Please enter the hostname of Fleet Server: " hostname_fleet
    read -p "Please enter the IP of Fleet Server: " ip_fleet
    read -p "Please enter the output file name for Fleet Server certs - Ex [fleet-certs.zip]: " certs_file_name
    # Chain 3 Create Certs
